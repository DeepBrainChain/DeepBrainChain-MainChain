...
/// An RPC endpoint to check for finality of blocks and transactions in Dbc
#[rpc(server)]
pub trait DbcFinalityApi {
    /// Reports whether a Substrate or Ethereum block is finalized.
    /// Returns false if the block is not found.
    #[method(name = "dbc_isBlockFinalized")]
    ...
    #[method(name = "dbc_isTxFinalized")]
    fn is_tx_finalized(&self, tx_hash: H256) -> RpcResult<bool>;
}

pub struct DbcFinality<B: Block, C> {
    pub backend: Arc<FrontierBackend<B>>,
    pub client: Arc<C>,
    _phdata: PhantomData<B>,
}

impl<B: Block, C> DbcFinality<B, C> {
    pub fn new(client: Arc<C>, backend: Arc<FrontierBackend<B>>) -> Self {
        Self { backend, client, _phdata: Default::default() }
    }
}

impl<B, C> DbcFinalityApiServer for DbcFinality<B, C>
where
    B: Block<Hash = H256>,
