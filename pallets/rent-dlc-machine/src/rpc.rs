...
impl<T: Config> Pallet<T> {
    pub fn get_dlc_rent_order(
        rent_id: RentOrderId,
    ) -> Option<RentOrderDetail<T::AccountId, T::BlockNumber, BalanceOf<T>>> {
        Self::rent_info(&rent_id)
    }

    pub fn get_dlc_rent_list(renter: T::AccountId) -> Vec<RentOrderId> {
        Self::user_order(&renter)
    }

    pub fn is_dlc_machine_renter(machine_id: MachineId, renter: T::AccountId) -> bool {
    ...
    pub fn get_dlc_machine_rent_id(machine_id: MachineId) -> MachineGPUOrder {
