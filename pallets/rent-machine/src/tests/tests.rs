...
fn rent_machine_should_works() {
    new_test_ext_after_machine_online().execute_with(|| {
        ...
        // So, rent fee: 59890 / 1000 * 5000000 / 12000 * 10 * 7 / 10 =  174679.16666666667 DBC
        ...
        let stash_machines = OnlineProfile::stash_machines(&*stash);
        assert_eq!(stash_machines.total_rent_fee, 174679166666666666666 * 2);
        assert_eq!(
            Balances::free_balance(*stash),
            INIT_BALANCE + 174679166666666666666 * 2 - 400000 * ONE_DBC
        );
        assert_eq!(
            Balances::free_balance(*renter_dave),
