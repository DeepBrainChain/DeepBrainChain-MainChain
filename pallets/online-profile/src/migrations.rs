use crate::{
    ...
    PendingSlash, StandardGPUPointPrice, StashMachines, StashStake, StorageVersion, SysInfo,
};
...
pub fn apply<T: Config>() -> Weight {
    ...
    if storage_version <= 1 {
        ...
        migrate_v2::migrate_machine_info_to_v2::<T>()
            .saturating_add(migrate_v2::migrate_pending_slash_to_v2::<T>())
    } else if storage_version == 2 {
        StorageVersion::<T>::put(3);
        migrate_v3::fix_slashed_online_machine::<T>() +
            migrate_v3::fix_online_rent_orders::<T>() +
            migrate_v3::regenerate_sys_info::<T>() +
...
pub mod migrate_v4 {
    ...
    impl<Balance, BlockNumber> From<OldUserMutHardwareStakeInfo<Balance, BlockNumber>>
        for UserMutHardwareStakeInfo<Balance, BlockNumber>
    where
        Balance: Default,
    {
        fn from(
            info: OldUserMutHardwareStakeInfo<Balance, BlockNumber>,
        ) -> UserMutHardwareStakeInfo<Balance, BlockNumber> {
            UserMutHardwareStakeInfo {
                verify_fee: info.stake_amount,
                offline_slash: Balance::default(),
                offline_time: info.offline_time,
                need_fulfilling: false,
            }
        }
    }

    pub fn migrate_user_mut_hardware_stake_info<T: Config>() -> Weight {
        UserMutHardwareStake::<T>::translate::<
            OldUserMutHardwareStakeInfo<BalanceOf<T>, T::BlockNumber>,
            _,
        >(|_, _, info| Some(info.into()));
        let count = UserMutHardwareStake::<T>::iter_values().count();

        info!(
            target: "runtime::onlineProfile",
            "migrated {} onlineProfile UserMutHardwareStakeInfo.",
            count,
        );
...
pub mod migrate_v2 {
    ...
    struct OldMachineInfo<AccountId: Ord, BlockNumber, Balance> {
    ...
    #[derive(PartialEq, Eq, Clone, Encode, Decode, Default, RuntimeDebug)]
    struct OldOPPendingSlashInfo<AccountId, BlockNumber, Balance> {
    ...
    impl<AccountId, BlockNumber, Balance>
        From<OldOPPendingSlashInfo<AccountId, BlockNumber, Balance>>
    ...
    pub fn migrate_machine_info_to_v2<T: Config>() -> Weight {
    ...
    pub fn migrate_pending_slash_to_v2<T: Config>() -> Weight {
...
pub mod migrate_v3 {
    ...
    pub fn fix_slashed_online_machine<T: Config>() -> Weight {
        let all_stash = <StashMachines<T> as IterableStorageMap<T::AccountId, _>>::iter()
            .map(|(stash, _)| stash)
            .collect::<Vec<_>>();
    ...
    pub fn fix_online_rent_orders<T: Config>() -> Weight {
    ...
    pub fn regenerate_sys_info<T: Config>() -> Weight {
        let all_stash = <StashMachines<T> as IterableStorageMap<T::AccountId, _>>::iter()
            .map(|(stash, _)| stash)
            .collect::<Vec<_>>();
        ...
        for stash in all_stash {
            ...
            if !stash_machine.online_machine.is_empty() {
                ...
                total_calc_points =
                    total_calc_points.saturating_add(stash_machine.total_calc_points);
    ...
    // 1.销毁达到2500卡启动
    // 2.单位算力值价格变更为60％
    pub fn reset_params<T: Config>() -> Weight {
        let mut standard_gpu_point_price =
            Pallet::<T>::standard_gpu_point_price().unwrap_or_default();
        standard_gpu_point_price.gpu_price = Perbill::from_rational_approximation(60u32, 100u32) *
