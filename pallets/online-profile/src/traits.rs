...
impl<T: Config> OCOps for Pallet<T> {
    ...
    fn confirm_machine(verify_committee: Vec<T::AccountId>, hardware_info: CommitteeUploadInfo) {
    ...
    // When committees reach an agreement to refuse machine,
    // change machine status and record refuse time
    fn refuse_machine(
