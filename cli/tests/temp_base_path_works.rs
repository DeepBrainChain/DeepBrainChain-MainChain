...
fn temp_base_path_works() {
    ...
    let mut cmd = cmd.args(&["--dev", "--tmp"]).stdout(Stdio::piped()).stderr(Stdio::piped()).spawn().unwrap();
    ...
    let db_path = PathBuf::from(re.captures(stderr.as_str()).unwrap().get(1).unwrap().as_str().to_string());
